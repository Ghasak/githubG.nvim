...
hl.common = {
    ...
    ToolbarLine = {fg = c.fg},
    Cursor = {bg = c.black, fg= c.blue, fmt = "reverse"},
    vCursor = {bg = c.black, fg= c.purple_e, mt = "reverse"},
    iCursor = {bg = c.black, fg= c.Charm_Pink, fmt = "reverse"},
    lCursor = {bg = c.black, fg= c.blue, fmt = "reverse"},
    CursorIM = {bg = c.black, fg= c.blue, fmt = "reverse"},
    --CursorIM = {bg = c.blue, fg= c.black, fmt = "reverse"},
...
function M.setup()
        ...
        if color_name:sub(1, 1) == '$' then
            ...
            if not color_name then
                vim.schedule(function()
                    vim.notify('onedark.nvim: unknown color "' .. name .. '"',
                               vim.log.levels.ERROR, {title = "onedark.nvim"})
                end)
                return ""
            end
        end
    ...
    for group_name, group_settings in pairs(vim.g.onedark_config.highlights) do
        vim.api.nvim_command(string.format("highlight %s %s %s %s %s",
                                           group_name, replace_color("guifg",
                                                                     group_settings.fg),
                                           replace_color("guibg",
                                                         group_settings.bg),
                                           replace_color("guisp",
